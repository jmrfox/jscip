...
def test_copy_bank():
    ...
    assert len(bank_copy.constraints) == len(bank.constraints)
    # copied bank must be able to sample and respect the constraint
    sample = bank_copy.array_to_instance(bank_copy.sample())
    assert sample["a"] > 0.5
...
def test_pretty_print(capsys):
    ...
    assert "is_sampled=True" in captured.out
    assert "(0.0, 2.0)" in captured.out
